...
class ExportHelper {
  static void exportToCsv(
    ...
    final anchor = web.document.createElement('a') as web.HTMLAnchorElement
      ..href = url
      ..download = "${title.replaceAll(' ', '_').toLowerCase()}.csv";
  ...
  static void exportToExcel(
    ...
    sheetObject.appendRow([
      ...
      TextCellValue(rows.length.toString()),
    ]);
    ...
    if (fileBytes != null) {
      final blob = web.Blob([Uint8List.fromList(fileBytes).toJS].toJS);
      final url = web.URL.createObjectURL(blob);
      final anchor = web.document.createElement('a') as web.HTMLAnchorElement
        ..href = url
        ..download = "${title.replaceAll(' ', '_').toLowerCase()}.xlsx";
      web.document.body?.appendChild(anchor);
      anchor.click();
      web.document.body?.removeChild(anchor);
      web.URL.revokeObjectURL(url);
  ...
  static String _formatDateRange(DateTime? startDate, DateTime? endDate) {
    ...
    if (startDate == null) {
      return 'Up to ${DateFormat('dd MMM yyyy').format(endDate!)}';
    }
    if (endDate == null) {
      return 'Since ${DateFormat('dd MMM yyyy').format(startDate)}';
    }
