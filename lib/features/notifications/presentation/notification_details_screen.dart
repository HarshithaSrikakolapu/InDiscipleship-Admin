...
class NotificationDetailsScreen extends ConsumerWidget {
          ...
          return SingleChildScrollView(
            ...
            child: Row(
              ...
              children: [
                ...
                Expanded(
                  flex: 1,
                  child: _buildInfoCard(notification),
                ),
              ],
            ),
          );
  ...
  Widget _buildHeaderCard(BuildContext context, WidgetRef ref, NotificationModel n) {
    return Card(
      elevation: 2,
          ...
          children: [
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(n.title, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  Text(n.message, style: const TextStyle(fontSize: 16, color: Colors.black87)),
                ],
              ),
            ),
            IntrinsicWidth(
              child: Column(
                ...
                if (n.status == NotificationStatus.draft || n.status == NotificationStatus.scheduled)
                  ElevatedButton.icon(
                    ...
                    label: Text(n.status == NotificationStatus.draft ? 'Edit Draft' : 'Edit Scheduled'),
                  ),
                const SizedBox(height: 8),
                ...
                OutlinedButton.icon(
                  onPressed: () {
                    ref.read(notificationControllerProvider.notifier).duplicateNotification(n.id, 'admin');
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Delivery Analytics', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const Divider(height: 32),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceAround,
              children: [
                _buildStat('Total Recipients', n.totalRecipients.toString()),
                _buildStat('Delivered', n.delivered.toString(), color: Colors.green),
                _buildStat('Failed', n.failed.toString(), color: Colors.red),
                _buildStat('Opened', n.opened.toString(), color: Colors.blue),
                _buildStat('Click Rate', '${(n.clickRate * 100).toStringAsFixed(1)}%', color: Colors.orange),
              ],
            ),
            const SizedBox(height: 24),
            if (n.deliveryDurationMs != null)
               Text('Delivery Duration: ${n.deliveryDurationMs} ms', style: const TextStyle(color: Colors.grey, fontStyle: FontStyle.italic)),
          ],
        ),
      ),
    );
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Payload Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const Divider(height: 32),
            if (n.imageUrl != null) ...[
              const Text('Image Attachment:', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey)),
              const SizedBox(height: 8),
              Image.network(n.imageUrl!, height: 150),
              const SizedBox(height: 16),
            ],
            if (n.deepLink != null) ...[
              const Text('Deep Link:', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey)),
              const SizedBox(height: 8),
              Text(n.deepLink!, style: const TextStyle(color: Colors.blue, decoration: TextDecoration.underline)),
            ] else 
              const Text('No extra payload attributes.', style: TextStyle(color: Colors.grey)),
          ],
        ),
      ),
    );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoRow('Created At', DateFormat('dd MMM yyyy, HH:mm').format(n.createdAt)),
            _buildInfoRow('Target Type', n.targetType.name.replaceAll('_', ' ').toUpperCase()),
            ...
            if (n.scheduledAt != null)
              _buildInfoRow('Scheduled For', DateFormat('dd MMM yyyy, HH:mm').format(n.scheduledAt!)),
            if (n.sentAt != null)
              _buildInfoRow('Sent At', DateFormat('dd MMM yyyy, HH:mm').format(n.sentAt!)),
          ],
        ),
      ),
    );
