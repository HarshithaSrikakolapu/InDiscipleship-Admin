...
class UserManagementRepositoryImpl implements UserManagementRepository {
  ...
  Stream<List<UserManagementViewModel>> getUsersStream() {
    ...
    return Rx.combineLatest2(
      usersStream,
      progressStream,
      (QuerySnapshot usersSnap, QuerySnapshot progressSnap) {
        final progressMap = {
          for (var doc in progressSnap.docs)
            doc.id: UserProgress.fromFirestore(doc.data() as Map<String, dynamic>, doc.id)
        };
        ...
        final mentorsMap = {
          for (var user in allUsers)
            if (user.isMentor && user.accountStatus == AccountStatus.active) user.uid: user
        };
  ...
  Stream<UserManagementViewModel?> getUserStreamById(String uid) {
    ...
    final progressStream = _firestore.collection('user_progress').doc(uid).snapshots();

    return Rx.combineLatest2(
      userStream,
      progressStream,
      (DocumentSnapshot userSnap, DocumentSnapshot progressSnap) async {
        if (!userSnap.exists || userSnap.data() == null) return null;
        
        final user = AppUser.fromFirestore(userSnap.data() as Map<String, dynamic>, userSnap.id);
        
        UserProgress? progress;
        if (progressSnap.exists && progressSnap.data() != null) {
          progress = UserProgress.fromFirestore(progressSnap.data() as Map<String, dynamic>, progressSnap.id);
        }

        AppUser? mentor;
        if (user.mentorId != null && user.mentorId!.isNotEmpty) {
          final mentorDoc = await _firestore.collection('users').doc(user.mentorId).get();
          if (mentorDoc.exists && mentorDoc.data() != null) {
            mentor = AppUser.fromFirestore(mentorDoc.data() as Map<String, dynamic>, mentorDoc.id);
          }
        }
  ...
  @override
  Future<void> bulkUpdateAccountStatus(List<String> uids, AccountStatus status) async {
  ...
  @override
  Future<void> updateUserProfile(String uid, {String? displayName, String? country, String? language}) async {
